...
public static class CreateWebPageResourceUsecase
{
    public static void Create(ScaffoldInput input, ProjectMetadata metadata)
    {
        ...
        var apiContent = GenerateEndpointsFactory.Generate(new(
            Input: input,
            Metadata: metadata));
        ...
        var moduleAlreadyExists = GenerateModuleUsecase.TryCreateApiModule(
            input: input,
            metadata: metadata,
            pathToModule: pathToModule,
            apiMappingMethodName: apiContent.MapMethodName);
